...
# Load dan play background music (looping)
pygame.mixer.music.load("asset/Epic Japanese Music – Samurai Warrior_1.mp3")
pygame.mixer.music.set_volume(0.4)
pygame.mixer.music.play(-1)  # Loop forever

sound_walk = pygame.mixer.Sound("asset/Footsteps Grass Sound Effect (HD)_1.mp3")
sound_walk.set_volume(1)

sound_attack = pygame.mixer.Sound("asset/sword slash (sound effects) __ mani creation ___1_1_1.mp3")
sound_attack.set_volume(0.7)  # Sesuaikan volume jika perlu

sound_hit = pygame.mixer.Sound("asset/Male Pain_Hurt Sound Effects - 8 Sounds_1_1_1.mp3")
sound_hit.set_volume(0.5)  # Bisa disesuaikan

sound_die = pygame.mixer.Sound("asset/Dead Rails Death Sound Effect_1_1.mp3")
sound_die.set_volume(0.8)
...
class Samurai:
    ...
    def move(self, keys):
        if self.isremote or self.is_dead or self.is_attacking:
                if hasattr(self, 'is_walking') and self.is_walking:
                    sound_walk.stop()  # Hentikan jika mati/diam
        ...
        if self.is_moving and not getattr(self, 'is_walking', False):
            sound_walk.play(-1)  # Mulai play loop
            self.is_walking = True
        elif not self.is_moving and getattr(self, 'is_walking', False):
            sound_walk.stop()  # Hentikan langkah saat berhenti
            self.is_walking = False
    ...
    if state:
        ...
        for pid, pdata in all_data.items():
            if pid == current_player.id:
                ...
                previous_hp = current_player.hp  
                current_player.hp = pdata["hp"]
                if pdata["hp"] < previous_hp:
                    sound_hit.play()
